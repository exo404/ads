...
class InsertionSort implements SortAlgorithm {
    ...
    public <T extends Comparable<T>> T[] sort(T[] items, final int startIndex, final int endIndex) {
        ...
        /**
     * Sorts a subarray of the given items using the standard Insertion Sort algorithm in descending order.
     *
     * @param items The items to be sorted
     * @param startIndex    The starting index of the subarray
     * @param endIndex    The ending index of the subarray (exclusive)
     * @param <T>   The type of elements in the items, which must be comparable
     * @return The sorted items in opposite order
     */
    public <T extends Comparable<T>> T[] sortOpposite(T[] items, final int startIndex, final int endIndex) {
        if (items == null || startIndex >= endIndex) {
            return items;
        }
        ...
        for (int i = startIndex + 1; i < endIndex; i++) {
            final T key = items[i];
            int j = i - 1;
            while (j >= startIndex && SortUtils.greater(key, items[j])) {
                items[j + 1] = items[j];
                j--;
            }
            items[j + 1] = key;
